...
# --- ESTILOS SEGUROS (V13.3) ---
st.markdown("""
    <style>
    /* 1. Ocultamos SOLO el menú de la derecha (Hamburguesa) */
    [data-testid="stToolbar"] {
        visibility: hidden;
        height: 0%;
    }
    
    /* 2. Ocultamos la decoración de colores superior */
    [data-testid="stDecoration"] {
        visibility: hidden;
        height: 0%;
    }

    /* 3. Ocultamos el footer "Made with Streamlit" */
    footer {
        visibility: hidden;
        height: 0%;
    }

    /* NOTA: Eliminamos los ajustes de 'stHeader' y 'block-container' 
       para que el botón del menú lateral (>) vuelva a su lugar original y funcione. */
...
def check_timeout():
    if st.session_state.logged_in:
        now = time.time()
        if (now - st.session_state.last_active) > TIMEOUT_SEGUNDOS: logout()
...
def get_gsheet_client():
    ...
    try: return gspread.authorize(ServiceAccountCredentials.from_json_keyfile_name("credentials.json", scope))
    except FileNotFoundError: return gspread.authorize(ServiceAccountCredentials.from_json_keyfile_dict(st.secrets["gcp_service_account"], scope))

# --- FUNCIONES NUBE ---
def sincronizar_inventario_descarga():
    try:
        client = get_gsheet_client(); sheet = client.open("PapeleriaDB").worksheet("Productos"); datos = sheet.get_all_records()
        c = conn.cursor(); c.execute("DELETE FROM productos") 
        if datos:
            for p in datos:
                if str(p['Codigo']) != "": c.execute("INSERT INTO productos (codigo_barra, nombre, precio, stock) VALUES (?, ?, ?, ?)", (str(p['Codigo']), p['Nombre'], float(p['Precio']), int(p['Stock'])))
            conn.commit(); st.session_state.ultima_sinc = hora_actual()
...
def guardar_producto_nube(codigo, nombre, precio, stock):
    try:
        client = get_gsheet_client(); sheet = client.open("PapeleriaDB").worksheet("Productos")
        sheet.append_row([str(codigo), nombre, precio, stock]); return True
    except: return False
...
def editar_producto_nube(codigo_original, nuevo_nombre, nuevo_precio, nuevo_stock):
    try:
        ...
        if cell:
            sheet.update_cell(cell.row, 2, nuevo_nombre); sheet.update_cell(cell.row, 3, nuevo_precio); sheet.update_cell(cell.row, 4, nuevo_stock)
            return True
        return False
    except: return False
...
def actualizar_stock_nube_lote(lista_cambios):
    try:
        client = get_gsheet_client(); sheet = client.open("PapeleriaDB").worksheet("Productos")
        todos = sheet.get_all_records(); batch = []
        ...
        if batch: sheet.batch_update(batch); st.session_state.ultima_sinc = hora_actual(); return True
        return False
    except: return False

def registrar_venta_nube_historial(fecha, ticket_id, vendedor, total, resumen):
    try:
        client = get_gsheet_client(); sheet = client.open("PapeleriaDB").worksheet("Ventas")
        sheet.append_row([str(fecha), ticket_id, vendedor, total, resumen]); return True
    except: return False

# --- LÓGICA ---
def login(u, p):
    df = pd.read_sql("SELECT * FROM usuarios WHERE nombre=? AND password=?", conn, params=(u,p))
    if not df.empty: st.session_state.logged_in=True; st.session_state.usuario_actual=df.iloc[0]['nombre']; st.session_state.rol_actual=df.iloc[0]['rol']; st.session_state.last_active = time.time(); st.rerun()
...
def scan_callback():
    ...
    if codigo:
        ...
        if not df.empty:
            prod = df.iloc[0]; cant = st.session_state.qty_scan
            if cant <= prod['stock']:
                 found = False
                 for item in st.session_state.carrito:
                     if item['id'] == prod['id']: item['cantidad'] += cant; item['subtotal'] = item['cantidad'] * item['precio']; found = True; break
                 if not found: st.session_state.carrito.append({"id": prod['id'], "codigo": prod['codigo_barra'], "nombre": prod['nombre'], "precio": prod['precio'], "cantidad": cant, "subtotal": cant*prod['precio']})
...
def procesar_venta_final(vendedor, pago):
    ...
    v_id = c.lastrowid
    resumen = ""; ticket = f"{NOMBRE_NEGOCIO}\n{UBICACION}\n\nTICKET #{v_id}\nFECHA: {fecha}\nATENDIÓ: {vendedor}\n{'-'*30}\n"; cambios_nube = []
    for item in st.session_state.carrito:
        c.execute("INSERT INTO detalle_ventas (venta_id, producto_nombre, cantidad, precio_unitario, subtotal) VALUES (?,?,?,?,?)", (v_id, item['nombre'], item['cantidad'], item['precio'], item['subtotal']))
...
if not st.session_state.inventario_sincronizado:
    with st.spinner("🔄 Iniciando..."): sincronizar_inventario_descarga()
...
if not st.session_state.logged_in:
    ...
    with st.sidebar:
        ...
        st.divider()
        if st.button("☁️ Forzar Recarga"): sincronizar_inventario_descarga(); st.rerun()
    ...
    if menu == "Punto de Venta":
        ...
        if st.session_state.carrito:
            for i, item in enumerate(st.session_state.carrito):
                ...
                c1.write(f"**{item['nombre']}**"); c2.write(f"${item['precio']}"); c3.write(f"x{item['cantidad']}"); c4.write(f"${item['subtotal']:.2f}")
            ...
            if st.button("✅ COBRAR", type="primary", use_container_width=True):
                if pago >= total:
                    ...
                    with c2: st.success("Venta Exitosa"); time.sleep(2); st.rerun()
                else: st.error("Faltan fondos")

    elif menu == "Reportes":
        st.subheader("📊 Finanzas")
        df_v = pd.read_sql("SELECT * FROM ventas", conn); df_d = pd.read_sql("SELECT * FROM detalle_ventas", conn)
        if not df_v.empty:
            k1, k2, k3 = st.columns(3)
            total_ing = df_v['total'].sum()
            k1.metric("Ingresos", f"${total_ing:,.2f}"); k2.metric("Tickets", len(df_v)); k3.metric("Promedio", f"${total_ing/len(df_v):,.2f}")
            st.divider(); c1, c2 = st.columns(2)
            with c1:
                st.markdown("##### 🏆 Top Productos")
                if not df_d.empty: st.bar_chart(df_d.groupby('producto_nombre')['cantidad'].sum().sort_values(ascending=False).head(5))
            with c2:
                st.markdown("##### 📅 Ventas Hora")
                df_v['fecha'] = pd.to_datetime(df_v['fecha'])
                st.line_chart(df_v.groupby(df_v['fecha'].dt.hour)['total'].sum())
            st.download_button("📥 Excel", to_excel(df_v), "reporte.xlsx")
        else: st.warning("Sin ventas.")

    elif menu == "Inventario":
        st.subheader("📦 Inventario Nube")
        if st.session_state.editando_id:
            p = pd.read_sql(f"SELECT * FROM productos WHERE id={st.session_state.editando_id}", conn).iloc[0]
            with st.form("edit"):
                nn=st.text_input("Nombre", value=p['nombre']); np=st.number_input("Precio", value=float(p['precio'])); ns=st.number_input("Stock", value=int(p['stock']))
                if st.form_submit_button("Guardar"):
                    if editar_producto_nube(p['codigo_barra'], nn, np, ns): sincronizar_inventario_descarga(); st.session_state.editando_id=None; st.rerun()
            ...
            with col_act1: prod_accion = st.selectbox("Seleccionar:", df['nombre'])
            with col_act2:
                st.write(""); st.write("")
